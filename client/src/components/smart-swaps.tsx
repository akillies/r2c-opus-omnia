...
import { Check, Box, DollarSign, Truck, Leaf, ArrowDown, ArrowUp, ArrowLeft, ArrowRight, Eye } from "lucide-react";
...
export default function SmartSwaps({ 
  ...
  const getSwapIcon = (swapType: string) => {
    switch (swapType) {
      case 'pack_size': return <Box className="w-4 h-4" />;
      case 'supplier': return <DollarSign className="w-4 h-4" />;
      case 'stock': return <Truck className="w-4 h-4" />;
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="font-bold text-gray-900 mb-1">Optimize Your Order</h3>
        <p className="text-xs text-gray-500">{swaps.length} swap opportunities found</p>
      </div>

      <div className="grid grid-cols-2 gap-2">
        <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-center">
          <div className="text-[10px] text-green-600 uppercase font-semibold tracking-wide">Potential Savings</div>
          <div className="text-lg font-bold text-green-900">${potentialSavings.toFixed(2)}</div>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 text-center">
          <div className="text-[10px] text-blue-600 uppercase font-semibold tracking-wide">Accepted</div>
          <div className="text-lg font-bold text-blue-900">{acceptedCount} / {swaps.length}</div>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-2.5">
                <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${getSwapColor(swap.swapType)}`}>
                  {getSwapIcon(swap.swapType)}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-1">
                    <h4 className="font-medium text-gray-900 text-sm leading-tight">{originalProduct.name}</h4>
                    {savings > 0 ? (
                      <Badge className="bg-green-100 text-green-700 text-[10px] px-1.5 py-0 shrink-0 flex items-center gap-0.5">
                        <ArrowDown className="w-2.5 h-2.5" />
                        ${savings.toFixed(2)}
                      </Badge>
                    ) : savings < 0 ? (
                      <Badge className="bg-amber-100 text-amber-700 text-[10px] px-1.5 py-0 shrink-0 flex items-center gap-0.5">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    {originalProduct.unitOfMeasure} → {recommendedProduct.unitOfMeasure}
                  </p>
                  <p className="text-xs text-gray-400 mt-0.5 line-clamp-1">{swap.reason}</p>
                  ...
                  {!swap.isAccepted ? (
                    <div className="flex items-center gap-2 mt-2">
                      <Button
                        ...
                        size="sm"
                        className="h-7 text-xs bg-[#1e3a5f] hover:bg-[#15293f] text-white px-3"
                    ...
                    <div className="flex items-center gap-1.5 mt-2 text-green-700">
                      ...
                      <span className="text-xs font-medium">Swap accepted</span>
                    </div>
                  )}
                </div>
              </div>
